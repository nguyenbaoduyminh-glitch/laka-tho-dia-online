export default async function handler(req, res) {
  ...
  try {
    const { prompt, city = "Đà Lạt", vibe = "CHILL", days = 3, budget = "2M" } = req.body || {};
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) return res.status(500).json({ error: "Missing GEMINI_API_KEY" });

    const finalPrompt = prompt || `Tạo lịch trình ${days} ngày tại ${city}, vibe ${vibe}, ngân sách ${budget}. Trả về JSON thuần.`;
